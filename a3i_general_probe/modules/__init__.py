...
def load_image_encoder(config_image_encoder: Dict):
    if config_image_encoder["source"].lower() == "huggingface":
        cache_dir = (
            config_image_encoder["cache_dir"]
            if "cache_dir" in config_image_encoder
            else "~/.cache/huggingface/hub"
        )
        gradient_checkpointing = (
            config_image_encoder["gradient_checkpointing"]
            if "gradient_checkpointing" in config_image_encoder
            else False
        )
        model_type = (
            config_image_encoder["model_type"]
            if "model_type" in config_image_encoder
            else "vit"
        )
        _image_encoder = HuggingfaceImageEncoder(
            ...
            local_files_only=os.path.exists(
                os.path.join(
                    cache_dir,
                    f'models--{config_image_encoder["name"].replace("/", "--")}',
                )
            ),
        )
    elif config_image_encoder["source"].lower() == "cnn" and (
        config_image_encoder["name"].lower() == "tf_efficientnet_b5_ns"
        or config_image_encoder["name"].lower() == "tf_efficientnetv2_s"
    ):
        _image_encoder = EfficientNet_Mammo(name=config_image_encoder["name"])
    elif (
        config_image_encoder["source"].lower() == "cnn"
        and config_image_encoder["name"].lower() == "tf_efficientnetv2-detect"
    ):
        _image_encoder = EfficientNet.from_pretrained("efficientnet-b2", num_classes=1)
        _image_encoder.out_dim = 1408
    elif (
        config_image_encoder["source"].lower() == "cnn"
        and config_image_encoder["name"].lower() == "tf_efficientnet_b5_ns-detect"
    ):
        _image_encoder = EfficientNet.from_pretrained("efficientnet-b5", num_classes=1)
        _image_encoder.out_dim = 2048
    elif config_image_encoder["source"].lower() == "cnn" and (
        config_image_encoder["name"].lower() == "resnet152"
        or config_image_encoder["name"].lower() == "resnet101"
    ):
    ...
    if config_text_encoder["source"].lower() == "huggingface":
        ...
        _text_encoder = HuggingfaceTextEncoder(
            ...
            local_files_only=os.path.exists(
                os.path.join(
                    cache_dir,
                    f'models--{config_text_encoder["name"].replace("/", "--")}',
                )
            ),
...
def load_projection_head(embedding_dim: int, config_projection_head: Dict):
    if config_projection_head["name"].lower() == "mlp":
        ...
        projection_head = LinearProjectionHead(
            embedding_dim=embedding_dim,
            projection_dim=config_projection_head["proj_dim"],
        )
    else:
...
def load_image_classifier(config_image_classifier: Dict, feature_dim: int):
    if config_image_classifier["name"].lower() == "linear":
        _image_classifier = LinearClassifier(
            feature_dim=feature_dim, num_class=config_image_classifier["n_class"]
        )
    else:
